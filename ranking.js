...
import { getDayKey, getWeekKey, safeWriteJson, jidToNumber } from './utils.js';

export const rankingCache = {}; // groupId -> { userId: stats }
...
export const persistDirtyGroups = () => {
  for (const groupId of Array.from(groupsToSave)) {
    ...
    try {
      fs.writeFileSync(filePath, JSON.stringify(rankingCache[groupId] || {}, null, 2), 'utf-8');
...
export const handleRankingListener = async ({ msg, isGroup, finalLid }) => {
  if (!isGroup) return;
  try {
...
export const handleRankingCommand = async ({ sock, msg, args, command, groupMetadata }) => {
  ...
  const currentWeek = moment().tz(TIMEZONE).format('YYYY-WW');
...
export const handleMyRankCommand = async ({ sock, msg, senderId }) => {
  ...
  if (!rankingCache[chatId]) {
    if (fs.existsSync(filePath)) {
      ...
      return sock.sendMessage(chatId, { text: '📊 No data recorded for this group yet.' }, { quoted: msg });
    }
  }
  const rankDb = rankingCache[chatId];
  ...
  if (myIndex > 0) {
    ...
    const diff = (userAbove.global - myCount) + 1;
